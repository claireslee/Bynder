...
public class Matches extends ReadURL {
    ...
    public Matches() throws Exception{
        for (int j = 0; j < name.size(); j++){
            people.add(new Person(name.get(j), age.get(j), hobbies1.get(j), hobbies2.get(j), movie.get(j), mbti.get(j), insideChar.get(j), island.get(j)));
        }
    }

    public String bestMatch1() {
       ...
       // String result = "\tBEST MATCH ONE\n";
        String result = "";
        for (int x = 0; x < b1.size(); x++) {
            result += people.get(x).getName();
            result += " matched with: ";
            result += b1.get(x).getName();
            result += "\n";
        }
    ...
    public String bestMatch2() {
            ...
            for (int j = 0; j < people.size(); j++) {
                if (i != j) {
                    ...
                    (people.get(i).getAge() == people.get(j).getAge() - 1)) {
                        ...
                        people.get(j).getMb().substring(0,4).equals("ENTP"))){
                            counter += 10;
                        }
       ...
       // String result = "\tBEST MATCH TWO\n";
       String result = "";
        for (int x = 0; x < b2.size(); x++) {
            result += people.get(x).getName();
            result += " matched with: ";
            result += b2.get(x).getName();
            result += "\n";
        }
        System.out.println(result);
        return result;
    ...
    public String worstMatch1() {
        ...
        //String result = "\tWORST MATCH ONE\n";
        String result = "";
        for (int x = 0; x < w1.size(); x++) {
            result += people.get(x).getName();
            result += " matched with: ";
            result += w1.get(x).getName();
            result += "\n";
        }
        System.out.println(result);
        return result;
